/**
 * Cloud SaaS Portal authentication utilities.
 * 
 * THIRD AUTH BOUNDARY - completely isolated from:
 *   - Admin Dashboard (NextAuth / getServerSession)
 *   - Connect Portal (connect_session / getConnectUser)
 * 
 * Uses its own JWT cookie ('cloud_session') and its own secret (CLOUD_JWT_SECRET).
 */
...
const rawSecret = process.env.CLOUD_JWT_SECRET || process.env.NEXTAUTH_SECRET;
if (!rawSecret) {
    console.warn(
        '[CloudAuth] ⚠️ WARNING: Neither CLOUD_JWT_SECRET nor NEXTAUTH_SECRET is configured. ' +
        'Using fallback key for compilation.'
    );
}

export const CLOUD_JWT_SECRET = new TextEncoder().encode(
    rawSecret || 'dummy-cloud-jwt-secret-key-for-build-only'
);
